...
for distcp purpose.
      ...
      options.hdfs_site["dfs.ha.namenodes.#{options.nameservice}"] = (for srv in service.use.hdfs_nn then srv.options.hostname).join ','
      for srv in service.use.hdfs_nn
      ...
      options.hdfs_site['dfs.namenode.shared.edits.dir'] = (for srv in service.use.hdfs_jn then "#{srv.node.fqdn}:#{srv.options.hdfs_site['dfs.journalnode.rpc-address'].split(':')[1]}").join ';'
